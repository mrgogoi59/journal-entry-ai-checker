          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-soft sm:p-6">
            ...
            <div className="mt-5 overflow-x-auto rounded-xl border border-blue-100">
              <table className="w-full min-w-[720px] border-collapse text-sm">
                <thead>
                  <tr className="bg-blue-950 text-left text-white">
                    {lesson.impactTable.columns.map((column) => (
                ...
                <tbody>
                  {lesson.impactTable.rows.map((row) => (
                    <tr key={`${row.first}-${row.second}-${row.third}`} className="border-b border-blue-50 last:border-b-0">
                      <td className="px-4 py-3 font-bold text-blue-950">{row.first}</td>
                      <td className="px-4 py-3 font-semibold leading-6 text-slate-800">{row.second}</td>
                      <td className="px-4 py-3 font-semibold leading-6 text-slate-800">{row.third}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
            {lesson.impactTable.note ? (
